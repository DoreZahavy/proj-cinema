...
function onCellClicked(elCell, i, j) {
    // console.log('gElSelectedSeat:', gElSelectedSeat)
    const cell = gCinema[i][j]
...
function showSeatDetails(pos) {
    ...
    var elLightBtn = document.querySelector('.btn-light')
    elLightBtn.dataset.i = pos.i
    elLightBtn.dataset.j = pos.j
...
function onBookSeat(elBtn) {
    ...
    var seatToLight = document.querySelector(`[title="Seat: ${i + 1}, ${j + 1}"]`)
    seatToLight.classList.add('booked')
...
function highlightNegs(elLightBtn) {
    const rowIdx = +elLightBtn.dataset.i
    const colIdx = +elLightBtn.dataset.j
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        ...
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            ...
            if (currCell.isSeat && !currCell.isBooked) {
                // gCinema[i][j].isLighted = true
                var seatToLight = document.querySelector(`[title="Seat: ${i + 1}, ${j + 1}"]`)
                seatToLight.classList.add('highlight')

            }
        }
    }
    // renderCinema()
    // const selectedSeat = document.querySelector(`[title="Seat: ${rowIdx + 1}, ${colIdx + 1}"]`)
    // selectedSeat.classList.add('selected')
    // onCellClicked(selectedSeat,rowIdx,colIdx)
    setTimeout(cleanHighlights, 2500, rowIdx, colIdx)
    // setTimeout(()=>{renderCinema()}, 2500)
}

function cleanHighlights(rowIdx, colIdx) {
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        if (i < 0 || i >= gCinema.length) continue
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            if (i === rowIdx && j === colIdx) continue
            if (j < 0 || j >= gCinema[0].length) continue
            var currCell = gCinema[i][j]
            // if (currCell.isLighted) {
            // gCinema[i][j].isLighted = false
            var seatToLight = document.querySelector(`[title="Seat: ${i + 1}, ${j + 1}"]`)
            // console.log('seatToLight:', seatToLight)
            seatToLight.classList.remove('highlight')
            // }
            // }
        }
